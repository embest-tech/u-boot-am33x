...
static void interface_clocks_enable(void)
{
	...
	while (readl(CM_PER_SPI1_CLKCTRL) != PRCM_MOD_EN);
...
static void power_domain_transition_enable(void)
{
	...
	writel(PRCM_FORCE_WAKEUP, CM_PER_L4FW_CLKSTCTRL);
...
static void per_clocks_enable(void)
{
	...
	/* UART3 */
	writel(PRCM_MOD_EN, CM_PER_UART3_CLKCTRL);
	while (readl(CM_PER_UART3_CLKCTRL) != PRCM_MOD_EN);
	...
	writel(PRCM_MOD_EN, CM_PER_CPSW_CLKSTCTRL);
	while ((readl(CM_PER_CPGMAC0_CLKCTRL) & 0x30000) != 0x0);
...
void enable_ddr2_clocks(void)
{
	...
	while ((readl(CM_PER_L3_CLKSTCTRL) & (PRCM_EMIF_CLK_ACTIVITY |
