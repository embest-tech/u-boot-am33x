...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	else if (board_is_bone_lt() || board_is_sbc8600())
...
void sdram_init(void)
{
	...
	else if (board_is_bone_lt() || board_is_som_ph8700() || board_is_sbc8600())
...
int board_eth_init(bd_t *bis)
{
	...
	} else if (board_is_som_ph8700() || board_is_sbc8600()) {
...
#define AR8051_RGMII_TX_CLK_DLY		0x100
	...
	if (board_is_evm_sk() || board_is_gp_evm() || board_is_sbc8600()) {
		const char *devname;
...
static struct am335x_lcdpanel lcdpanels[] = {
	{
		.hactive	= 480,
		.vactive	= 272,
		.bpp		= 32,
...
static int load_lcdtiming(struct am335x_lcdpanel *panel)
{
	...
	if (idx >= ARRAY_SIZE(lcdpanels)) {
		printf ("Warning: %s: idx=%d out of range, force idx=%d\n", __func__, idx, idx_default);
		idx = idx_default;
	}

	memcpy(panel, &lcdpanels[idx], sizeof(struct am335x_lcdpanel));
	if (panel->pxl_clk_div != 0)
		panel->pxl_clk_div = 192000000UL / panel->pxl_clk_div;
...
{
	int i;
	unsigned int period_ms = 1000 / freq;
