			 ...
			 struct ti_powerproc_privdata *priv)
{
	...
	priv->loadaddr = fdtdec_get_addr(blob, 0, "reg");
	if (priv->loadaddr == FDT_ADDR_T_NONE) {
		debug("'%s': no 'reg' property\n", dev->name);
		return -EINVAL;
	}

	tmp = fdtdec_get_int(blob, 0, "ti,psc_module", -EINVAL);
	if (tmp < 0) {
		debug("'%s': no 'ti,psc_module' property\n", dev->name);
		return tmp;
	}
	priv->psc_module = tmp;
...
#else
static inline int ti_of_to_priv(struct udevice *dev,
				struct ti_powerproc_privdata *priv)
{
	return -EINVAL;
}
...
/**
 * ti_plat_to_priv() - platform data to private data.
 * @dev:	corresponding ti remote processor device
 * @pdata:	platform data corresponding to device
 * @priv:	pointer to driver specific private data
 *
 * NOTE: XXX: all new platforms should ideally migrate to devicetree.
 * Return: 0
 */
static int ti_plat_to_priv(struct udevice *dev,
			   struct ti_powerproc_platdata *pdata,
			   struct ti_powerproc_privdata *priv)
{
	priv->loadaddr = pdata->slave_base;
	priv->psc_module = pdata->module;
	return 0;
}
...
static int ti_powerproc_probe(struct udevice *dev)
{
	...
	if (!pdata)
		...
		ret = ti_plat_to_priv(dev, pdata, priv);
